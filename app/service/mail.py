...
class EmailService:
    ...
    def send_message(self, name: str, email: str, phone: str, message: str) -> None:
        try:
            with smtplib.SMTP(
                self.config.get_param("SMTP_SERVER"),
                int(self.config.get_param("SMTP_PORT")),
            ) as server:
                ...
                server.starttls()
                server.login(
                    self.config.get_param("EMAIL_USERNAME"),
                    self.config.get_param("EMAIL_PASSWORD"),
                )
                server.sendmail(
                    self.config.get_param("SENDER_EMAIL"),
                    self.config.get_param("RECIPIENT_EMAIL"),
                    message.as_string(),
                )
                server.close()
    ...
    def _format_message(self, name: str, email: str, phone: str, message: str) -> str:
        with open(
            os.path.join(os.getcwd(), "app", "template", "mail.html"), "r"
        ) as file:
