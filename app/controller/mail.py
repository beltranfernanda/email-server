...
class EmailController:
    ...
    def send_mail(self) -> Union[tuple[Response, HTTPStatus], Response]:
        ...
        if not is_valid_body:
            response = ApiResponse(400, f"Field {field} is required").get_formatted_response()
            return jsonify(response), http.HTTPStatus.BAD_REQUEST

        try:
            self.email_service.send_message(body["name"], body["email"], body.get("phone", ""), body["message"])
            response = ApiResponse(200, "Everything works fine!").get_formatted_response()
            ...
            response = ApiResponse(500, f"Internal server error: {err}").get_formatted_response()
